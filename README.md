...
The set of options can be displayed by running ``cluster -h``, they are essentially the same as the ones for ``match`` routine (except for database-related options). Output file is a tab-delimited table that contains pairs of clonotypes (as in sample), their IDs and distances between them. Scoring and filtering options can be used in the same way as with ``match`` routine.
