...
(per-epitope mean, robust to the $100\times$ epitope-size range; \S below) $0.44,\,0.23,\,0.14,\,0.10,
\,0.07$ at distance $1\ldots5$, an enrichment of $44\times\!\to\!2.3\times$ over chance
(Fig.~\ref{fig:purity}). A distance-1 neighbour is twice as likely to be a true co-specific as a
distance-2 one and six times a distance-5 one. This reproduces the original VDJdb
...
dips to $\approx0.31$ in the NDN core but rises to $\approx0.90$ toward both the V and J borders: a
central mismatch most often \emph{changes} specificity (it is contact-proximal and informative), while
a border mismatch is usually germline noise (the conserved V/J stems). This is the empirical basis for
...
\emph{matches} BLOSUM62 ($+0.013$ at distance~$\le2$, $+0.016$ at $\le4$; ahead in $4$--$5/8$ epitopes)
...
$\approx1.9$, V/J borders $\approx0.2$--$0.4$), and interpolate it to any CDR3 length. \texttt{seqtree}
consumes this natively: \texttt{regions.significance\_pssm()} builds a fixed-width
\texttt{PositionalMatrix.from\_weights(BLOSUM62, $\lfloor100\,\omega(p)\rceil$)}, so the engine scores
$\mathrm{pen}(p,a,b)=\omega(p)\cdot\mathrm{BLOSUM62}(a,b)$ --- a central mismatch costs $\sim2\times$
its border-position cost. This is the only scoring change that clearly beats flat BLOSUM62:
$0.572\!\to\!0.623$ at distance~$\le2$ (winning $7/8$ epitopes) and $0.589\!\to\!0.617$ at $\le4$
($6/8$) (Table~\ref{tab:matrices}). The gain is uniformly signed because the matrix re-ranks within a
ball whose purity is already set by the radius. The remaining lever, the \emph{V-gene} dimension, is
taken up in~\S\ref{sec:vgene}.
...
\texttt{bench/tcrblosum\_refute.py}), it does \emph{not} beat BLOSUM62: $0.557$ vs $0.570$ at
distance~$\le2$ (ahead in $3/8$ epitopes), $0.580$ vs $0.586$ at $\le4$ ($5/8$), while their bundled
BLOSUM62 reproduces ours exactly (a loader sanity check). Reweighting \emph{either} matrix by the
central-significance PSSM helps, and it helps BLOSUM62 the most (BLOSUM$+$possig $0.622$ vs
tcrBLOSUM$+$possig $0.617$). The lesson is consistent: a re-counted amino-acid alphabet does not
...
\hline
edit distance $\le2$ & 0.545 & 0.572 & 0.575 & 0.572 & 0.519 & 0.585 & 0.525 & \textbf{0.623}\\
edit distance $\le4$ & 0.576 & 0.589 & 0.597 & 0.589 & 0.535 & 0.605 & 0.544 & \textbf{0.617}\\
\hline
\end{tabular}
...
\paragraph{But near-\emph{exact} germline identity does --- at CDR3-like tolerance.} Resolving the
similarity to exact germline-region matches reverses that impression
(\texttt{bench/vregion\_decompose.py}, human TRB MHC-I; Table~\ref{tab:vregion}). Cross-V
co-specificity rises \emph{monotonically} as the germline CDR1$+$CDR2 edit distance shrinks --- $10.9\%$
at $\ge6$ mismatches, $13.7\%$ at $3$--$5$, $20.6\%$ at $1$--$2$, and $60.0\%$ at $0$ (identical loops),
approaching the same-V level ($65\%$). FR1 identity behaves likewise ($50\%$, $4.4\times$). So the V
signal \emph{is} carried by the germline contacting loops, but with a tight, CDR3-like tolerance: it
needs near-exact identity, which is why the continuous metric above (dominated by $1$--$2$-mismatch
pairs) misses it. Caveat: the exact-match bins are small ($n=25$ at distance~$\le2$, $n=4$ at $\le1$);
the claim rests on the monotone trend across the well-powered bins ($n=568,\,3679,\,34095$), with the
identical-loop endpoint suggestive.
...
baseline), with a single exception --- CDR2 position~5 ($1.67\times$, $n=5392$). So the prior is
recovered by \emph{whole-loop} near-identity, not by matching a handful of pseudosequence coordinates:
...
\hline
same V  & 116\,919 & 44.1\% & 0.632 & 0.713 & 0.680\\
cross V & 981\,672 &  6.3\% & 0.560 & 0.573 & 0.537\\
\hline
\end{tabular}
...
significance PSSM helps the same-V stratum most ($+0.081$).}
\label{tab:vgene}
\end{table}
...
($6\%\!\to\!12\%$), far below the same-V $44\%$ --- which is why $r\approx0$. The strong signal lives at
near-\emph{exact} identity (Table~\ref{tab:vregion}), not along the continuous similarity axis.}
\label{tab:vgene-sim}
\end{table}
...
\hline
human TRA MHC-I  & 57.9\% & 17.3\% & 3.3 & $0.060$\\
human TRA MHC-II & 98.0\% & 24.8\% & 4.0 & $0.242$\\
human TRB MHC-I  & 64.3\% &  8.0\% & 8.0 & $0.056$\\
human TRB MHC-II & 97.5\% & 76.1\% & 1.3 & $0.046$\\
mouse TRA MHC-I  & 75.1\% & 29.3\% & 2.6 & $0.021$\\
mouse TRA MHC-II & 50.4\% & 48.0\% & 1.0 & $-0.010$\\
mouse TRB MHC-I  & 76.7\% & 40.6\% & 1.9 & $-0.030$\\
mouse TRB MHC-II & 83.7\% & 31.6\% & 2.7 & $-0.016$\\
\hline
\end{tabular}
...
\hline
\emph{cross-V baseline}        & 11.4\% & 38\,419\\
CDR1$+$CDR2 edit distance $\ge6$ & 10.9\% & 34\,095\\
\quad edit $3$--$5$            & 13.7\% &  3\,679\\
\quad edit $1$--$2$            & 20.6\% &    568\\
\quad edit $0$ (identical)     & \textbf{60.0\%} &  25\\
FR1 identical                  & 50.3\% &   338\\
\emph{same-V reference}        & 65.2\% & 12\,869\\
\hline
\end{tabular}
\caption{Cross-V co-specificity rises monotonically as germline CDR1$+$CDR2 approach identity,
reaching $\approx$the same-V level at exact identity (edit~$0$); FR1 identity behaves likewise. Near-
\emph{exact} germline-loop identity --- not loose similarity --- recovers the V prior (edit~$0$/CDR1\&CDR2
bins small, $n=25$).}
\label{tab:vregion}
\end{table}
...
\hline
TRA & shortlist ($\ge2$ ref) & 1993 & 43.7\% & 43.7\% & 62.3\%\\
TRA & control ($1$ ref)      & 1993 & 32.2\% & 27.9\% & 48.4\%\\
TRB & shortlist ($\ge2$ ref) & 2962 & 47.2\% & 46.9\% & 54.2\%\\
TRB & control ($1$ ref)      & 2962 & 45.1\% & 41.5\% & 48.8\%\\
\hline
\end{tabular}
...
co-specificity does rise to $\approx$the same-V level at \emph{near-exact} CDR1$+$CDR2 identity --- a
tight, CDR3-like tolerance, recovered whole-loop rather than via a sparse pseudosequence (exact-match
bins small, $n\!\approx\!25$). The open work is to model V as a strong prior in a joint V$+$CDR3
