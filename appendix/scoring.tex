...
weighting the centre more --- exactly what the germline-retention weight $1-w(p)$ does
(\S\ref{sec:vdjam}) --- and it validates the direction even though, per Fig.~\ref{fig:loo}, it moves
retrieval little (the variation is already central). It also matches the contacting-region picture of
...
\paragraph{No matrix beats BLOSUM62; can we design one?} Re-scoring the held-out-epitope candidates
(leave-one-out, \S above) with each substitution matrix gives the same verdict at every scope
(Table~\ref{tab:matrices}): BLOSUM62 and PAM250 are the best and essentially tied, the structural
matrix sits between them and plain Hamming, Hamming (unit) is a strong baseline, and the data-derived
NDN-VDJAM --- flat or region-weighted --- trails them all (region beats flat by a small, consistent,
scope-growing margin). So the honest answer to ``can we beat BLOSUM62'' is, so far, no: a
substitution matrix is a second-order lever once the ball radius is fixed, and the first-order
statistic remains the control-calibrated neighbour count at distance~1 (\S\ref{sec:agg}). The one
regime still worth pushing is the \emph{V-gene} dimension (cross-V comparisons and V-similarity
grouping), where germline-flank differences are common and the central weighting should matter most.

\begin{table}[ht]\centering\small
\begin{tabular}{lcccccc}
\hline
mean retrieval PR-AUC & unit & BLOSUM62 & PAM250 & structural & VDJAM & VDJAM-region\\
\hline
edit distance $\le2$ & 0.288 & \textbf{0.323} & 0.321 & 0.301 & 0.269 & 0.275\\
edit distance $\le4$ & 0.207 & \textbf{0.223} & 0.222 & --- & 0.179 & 0.184\\
\hline
\end{tabular}
\caption{Substitution matrices compared by leave-one-out-by-epitope VDJdb-vs-VDJdb retrieval (TRB,
8 largest epitopes, micro PR-AUC). BLOSUM62 $\approx$ PAM250 lead at both radii; the data-derived
VDJAM trails; region weighting consistently beats flat VDJAM.}
\label{tab:matrices}
\end{table}
...
scope, does not beat BLOSUM62; the region-aware germline$+$trimming weighting (now implemented) gives a
small, consistent, scope-growing gain over the flat matrix but not over BLOSUM62 at this task. The
clearest remaining lever is the \emph{V-gene} dimension: a V-gene similarity term (grouping V genes by
CDR1/CDR2/CDR2.5 distance) and cross-V comparisons, where germline-flank differences are common and the
region weighting should matter most; a per-chain treatment (TRA vs TRB) is also pending. Separately, the
