...
\paragraph{Why BLOSUM/PAM do not transfer.} BLOSUM is a log-odds matrix estimated from gapless columns
of a trusted multiple alignment of homologous blocks. CDR3s violate both premises: (i) same-specificity
CDR3s differ in length and have no column homology, so there is no multiple alignment; (ii) the
germline-encoded ends are over-conserved --- essentially every human TRB CDR3 begins \texttt{CASS} and
ends in a short J motif (\texttt{\dots F}), so an alanine near the N-terminus is germline-fixed and
carries no specificity information, while an alanine in the centre does. A single global matrix
estimated over all positions conflates germline conservation with selection.
...
\paragraph{Paired $\alpha\beta$ E-value.} For a paired query the chains are independent under the null,
so the joint ball mass factorizes, $\pi_0^{\alpha\beta}\approx\pi_0^\alpha\pi_0^\beta$, and among the
$N$ paired VDJdb complexes the expected number of chance joint matches is $\E_{\alpha\beta}=N\,
\pi_0^\alpha\pi_0^\beta$ with $p$ the Poisson tail on the count of complexes matched in \emph{both}
chains. The joint null is far smaller than either chain's, so true pairs are dramatically more
significant: on the paired TCRvdb benchmark, $296/300$ pairs match a complex and $295/296$ ($99.7\%$)
recover the correct epitope, with joint $p$ down to $3.6\times10^{-9}$.
